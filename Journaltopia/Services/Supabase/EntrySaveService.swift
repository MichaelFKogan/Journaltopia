...
enum EntryCloudSyncFailureStore {
    private static let storageKey = "JournaltopiaEntryCloudSyncFailures"
        ...
        while true {
            do {
                ...
                guard attempt < maxAttempts, isTransient(error) else {
                    if attempt > 1 {
                        print("[Journaltopia] \(context) failed after \(attempt) attempt(s): \(error.localizedDescription)")
                    }
                    throw error
                }
                ...
                print("[Journaltopia] \(context) attempt \(attempt) hit a transient failure, retrying in \(String(format: "%.2f", delay))s: \(error.localizedDescription)")
...
struct SupabaseEntryThumbnailService {
    ...
    private let bucketName = "journaltopia-media"
    ...
    func deleteThumbnail(storagePath: String?) async {
        ...
        do {
            ...
            print("[Journaltopia] Entry thumbnail delete skipped: \(error.localizedDescription)")
        }
    }
...
struct SupabaseStoryboardService {
    ...
    func persistStoryboard(_ storyboard: GeneratedStoryboard) async throws -> EntryStoryboard {
        ...
        print("[Journaltopia] Cloud storyboard upload started.")
        do {
            try await client.storage
                .from(bucketName)
                .upload(
    ...
    func setPrimaryStoryboard(id storyboardID: UUID, clientEntryID: UUID) async throws {
        ...
        do {
            ...
            try await client
                ...
                .execute()
            print("[Journaltopia] Primary storyboard selection updated.")
        } catch let error as SupabaseStoryboardError {
            throw error
        } catch {
            print("[Journaltopia] Primary storyboard update failed: \(error.localizedDescription)")
            throw SupabaseStoryboardError.syncFailed
        }
    }
    ...
    func deleteStoryboards(ids: Set<UUID>, clientEntryID: UUID) async throws -> [EntryStoryboard] {
        ...
        do {
            ...
            print("[Journaltopia] Storyboard delete succeeded for \(doomedRows.count) storyboard(s).")
            return rows.filter { !ids.contains($0.id) }
        } catch let error as SupabaseStoryboardError {
            throw error
        } catch {
            print("[Journaltopia] Storyboard delete failed: \(error.localizedDescription)")
            throw SupabaseStoryboardError.syncFailed
        }
    }
    ...
    private func downloadStoryboardImages(from rows: [EntryStoryboard]) async -> [GeneratedStoryboard] {
        ...
        for row in rows {
            do {
                ...
                print("[Journaltopia] Profile storyboard image download skipped: \(row.id) \(error.localizedDescription)")
            }
        }
    ...
    func downloadStoryboardImage(storagePath: String) async throws -> UIImage {
        do {
            ...
            if let cachedData = SupabaseStorageImageCache.data(bucketName: bucketName, storagePath: storagePath) {
                ...
                print("[Journaltopia] Cloud image download/cache miss.")
                data = try await client.storage
                    .from(bucketName)
                    .download(path: storagePath)
                SupabaseStorageImageCache.store(data, bucketName: bucketName, storagePath: storagePath)
    ...
    private func markPriorStoryboardsNonPrimary(
        ...
        try await client
            ...
            .execute()
        print("[Journaltopia] Prior primary versions updated.")
    }
...
struct EntrySaveService {
    ...
    func saveEntryPreservingStatus(
        ...
        if status == .completed {
            print("[Journaltopia] Local entry marked completed.")
        }
        ...
        if status == .completed {
            print("[Journaltopia] Supabase status update started.")
        }
        print("[Journaltopia] Supabase entry save payload hasLocation=\(hasLocation), datePrecision=\(payload.datePrecision.rawValue), sendsEntryDate=\(cloudEntryDate != nil).")
        ...
        if let thumbnail = CreateEntryDraftStore.load(id: localDraftID)?.thumbnail {
            do {
                ...
                print("[Journaltopia] Entry thumbnail sync failed: \(error.localizedDescription)")
            }
        }
        ...
        guard syncReferencePhotos else {
            print("[Journaltopia] Reference photos unchanged, sync skipped.")
            return EntrySaveResult(
                localDraftID: localDraftID,
                cloudEntry: cloudEntry,
                state: .saved
    ...
    private func notSavedResult(
        ...
        print("[Journaltopia] Cloud entry save gave up, entry is local only: \(error.localizedDescription)")
    ...
    func prepareEntryForGeneration(
        ...
        print("[Journaltopia] Generation preparation started.")
        ...
        guard requiresSave else {
            ...
            print("[Journaltopia] Entry was clean, cloud save skipped.")
            print("[Journaltopia] Reference photos unchanged, sync skipped.")
            return EntrySaveResult(
                localDraftID: localDraftID,
                cloudEntry: nil,
        ...
        print("[Journaltopia] Entry was dirty, cloud save started.")
    ...
    func markEntryCompletedAfterStoryboardSaved(
        ...
        if case .failed = result.state {
            print("[Journaltopia] Supabase status update failed.")
        } else if case .notSaved = result.state {
            print("[Journaltopia] Supabase status update failed.")
        } else if isSignedIn {
            print("[Journaltopia] Supabase status update succeeded.")
        }
        return result
    }
