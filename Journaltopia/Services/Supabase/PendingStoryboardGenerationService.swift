...
extension GeneratedStoryboardStore {
    private static var pendingGenerationKey: String {
        JournaltopiaLocalAccountScope.scopedUserDefaultsKey("JournaltopiaPendingStoryboardGenerations")
    }
...
struct PendingStoryboardGenerationService {
    ...
    private func reconcileCompletedGeneration(
        ...
        guard let storagePath = row.storagePath else {
            ...
            print("[Journaltopia] Completed storyboard row has no storage path: \(pendingGeneration.id)")
        ...
        if let alreadyPersisted = locallyPersistedStoryboard(pendingGeneration) {
            ...
            do {
                ...
                print("[Journaltopia] Pending storyboard download failed: \(pendingGeneration.id) \(error.localizedDescription)")
                return .waiting(
                    PendingStoryboardGenerationProgress(
                        state: .completed,
            ...
            do {
                ...
                print("[Journaltopia] Pending storyboard could not be stored locally: \(pendingGeneration.id) \(error.localizedDescription)")
    ...
    private func markEntryCompleted(clientEntryID: UUID) async -> Bool {
        ...
        do {
            ...
            print("[Journaltopia] Pending storyboard entry completion deferred: \(clientEntryID) \(error.localizedDescription)")
            return false
        }
    }
...
final class PendingStoryboardGenerationMonitor: ObservableObject {
    ...
    private func pollOnce(_ outstanding: [PendingStoryboardGeneration]) async {
        ...
        do {
            ...
            print("[Journaltopia] Pending storyboard status check failed: \(error.localizedDescription)")
            return
        }
        ...
        for (pendingGeneration, outcome) in resolved {
            ...
            case .vanished:
                reconciledIDs.insert(pendingGeneration.id)
                print("[Journaltopia] Pending storyboard row no longer exists: \(pendingGeneration.id)")
            }
    ...
    private func waitingStatus(
        ...
        if progress.isStalled {
            message = "This is taking longer than usual. Journaltopia is still waiting on the server."
        } else if isRestored {
            message = "Still generating on Journaltopia's servers. This keeps running if you leave."
        } else {
