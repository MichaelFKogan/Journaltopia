...
struct StoryboardDeletionService {
    ...
    func delete(_ storyboards: [GeneratedStoryboard], isSignedIn: Bool) async throws -> StoryboardDeletionOutcome {
        ...
        for (clientEntryID, storyboardIDs) in grouped {
            do {
                ...
                print("[Journaltopia] Storyboard delete failed for entry \(clientEntryID): \(error.localizedDescription)")
                failedEntryCount += 1
            }
        }
    ...
    private func deleteStoryboards(
        ...
        guard let newestRemaining = remaining.max(by: { $0.createdAt < $1.createdAt }) else {
            ...
            print("[Journaltopia] Entry \(clientEntryID) returned to drafts after its last storyboard was deleted.")
            return
        }
    ...
    private func existingStoryboardIDsByEntry(
        ...
        if isSignedIn {
            do {
                ...
                print("[Journaltopia] Storyboard deletion preview fell back to local counts: \(error.localizedDescription)")
            }
        }
