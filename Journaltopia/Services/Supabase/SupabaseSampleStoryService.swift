...
struct SupabaseSampleStoryService {
    ...
    private let authoringPackSlug = "journaltopia-first-run"
    ...
    /// Sample storyboards generated by the generate-sample-storyboard function are written server
    /// side, so the studio has to drop its cached pack to pick them up.
    func invalidateSamplePackCache() {
        SampleStoryPackCache.clear()
    }
    ...
    func deleteSampleEntry(id: UUID) async throws {
        ...
        do {
            ...
            print("[Journaltopia] Sample entry RPC delete failed; trying direct delete. \(error.localizedDescription)")
            deletedRows = try await deleteSampleEntryDirectly(id: id)
        }

        guard deletedRows.contains(where: { $0.id == id }) else {
            print("[Journaltopia] Sample entry delete affected no rows for \(id).")
            throw JournalEntryRepositoryError.operationFailed
        }
    ...
    private func loadSampleAssets(entryIDs: [UUID]) async -> [UUID: SampleEntryLoadedAssets] {
        ...
        do {
            ...
            print("[Journaltopia] Sample entry asset load failed: \(error.localizedDescription)")
            return [:]
        }
    }
        ...
        for page in pages {
            ...
            storyboardsByEntryID[page.sampleEntryID, default: []].append(
                GeneratedStoryboard(
                    ...
                    artStyle: page.artStyle ?? entry.artStyle ?? "Anime",
    ...
    private func uploadEntryThumbnail(_ thumbnail: UIImage?, sampleEntryID: UUID) async throws {
        guard
            let thumbnail,
            let imageData = thumbnail.journaltopiaPreparedJPEGData(compressionQuality: 0.85)
        else {
            return
        }
    ...
    private func deleteExistingSampleMediaRows(entryID: UUID) async throws -> [SampleEntryAssetRow] {
        ...
        guard remainingRows.isEmpty else {
            ...
            print("[Journaltopia] Sample asset delete verification failed for entry \(entryID); remaining rows: \(remainingDescription)")
            throw JournalEntryRepositoryError.operationFailed
        }
    ...
    private func deduplicatedSamplePhotos(_ photos: [CreateEntryReferencePhoto]) -> [CreateEntryReferencePhoto] {
        ...
        return photos.filter { photo in
            guard let data = photo.image.journaltopiaPreparedJPEGData(compressionQuality: 0.9) else {
                return true
            }
    ...
    private func deduplicatedSampleCharacters(_ characters: [EntryCharacter]) -> [EntryCharacter] {
        ...
        return characters.filter { character in
            guard let data = character.image.journaltopiaPreparedJPEGData(compressionQuality: 0.9) else {
                return true
            }
