...
///    `Storytopia`, so a store added next month is erased on sign-out the day it is written, with
...
///    survive on purpose.
/// 2. **Directories are removed whole.** `StorytopiaAccounts` goes at its root, so every account
...
enum LocalUserDataPurge {
    ...
    static let devicePreferenceKeys: Set<String> = [
        ...
        "StorytopiaSelectedEntryLayout",
        "StorytopiaSelectedEntrySort",
        "StorytopiaImageGenerationQuality",
        "StorytopiaEntryThumbnailRendererVersion",
    ...
    /// The prefix every Storytopia-written default shares, including the `.<scope-id>` suffixed
    /// forms that `StorytopiaLocalAccountScope` produces and the unscoped legacy keys that predate
    /// scoping.
    static let userDefaultsKeyPrefix = "Storytopia"

    /// Folders under `Documents` that hold account content.
    ///
    /// `StorytopiaAccounts` is the scoped tree every current store writes into. The rest are the
    ...
    static func purgeAll() {
        for step in steps {
            do {
                ...
                print("[Storytopia] Sign-out purge step '\(step.name)' failed: \(error.localizedDescription)")
            }
        }
    }
    ...
    /// living in `UserDefaults` under the `Storytopia` prefix, nowhere at all.
    ...
    /// Removes every `Storytopia`-prefixed default that is not a device preference.
    ///
    ...
    static func shouldPurgeUserDefaultsKey(_ key: String) -> Bool {
        guard key.hasPrefix(userDefaultsKeyPrefix) else {
            return false
        }

        return !devicePreferenceKeys.contains(unscopedBaseKey(for: key))
    }
