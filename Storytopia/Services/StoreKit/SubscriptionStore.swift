...
final class SubscriptionStore: ObservableObject {
    ...
    func restorePurchases(isSignedIn: Bool) async -> Bool {
        ...
        do {
            ...
            print("[Storytopia] AppStore.sync() did not complete: \(error)")
        }
    ...
    private func handle(_ update: VerificationResult<Transaction>) async {
        guard let transaction = verifiedTransaction(from: update) else {
            print("[Storytopia] Ignoring an unverified StoreKit transaction.")
            return
        }
    ...
    private func verifiedTransaction(from result: VerificationResult<Transaction>) -> Transaction? {
        ...
        case .unverified(_, let error):
            print("[Storytopia] StoreKit verification failed: \(error.localizedDescription)")
            return nil
        }
    }
}
