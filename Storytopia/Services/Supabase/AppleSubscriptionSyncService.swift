...
struct AppleSubscriptionSyncService {
    ...
    func sync(
        ...
        guard (200..<300).contains(httpResponse.statusCode) else {
            ...
            print("[Storytopia] Apple subscription sync failed: \(httpResponse.statusCode) \(failure?.error ?? "")")
            throw AppleSubscriptionSyncError.unavailable
        }
        ...
        do {
            ...
            print("[Storytopia] Apple subscription sync response unreadable: \(error)")
            throw AppleSubscriptionSyncError.unavailable
        }
    }
}
...
struct JournaltopiaPlusEntitlementService {
    ...
    func fetchEntitlement() async throws -> JournaltopiaPlusState {
        ...
        let rows: [EntitlementRow] = try await client
            .from("storytopia_plus_entitlement")
            .select()
            .limit(1)
            .execute()
            .value
