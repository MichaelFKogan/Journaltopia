...
final class SignedOutRoutingTests: XCTestCase {
    ...
    func testSignedOutStatusBrowsesSamples() {
        let mode = StorytopiaContentMode(status: .signedOut, isSampleAuthorModeEnabled: false)
    ...
    func testSampleAuthorToggleWithoutAccountStillBrowses() {
        let mode = StorytopiaContentMode(status: .signedOut, isSampleAuthorModeEnabled: true)
    ...
    func testSignedInWithSampleAuthorModeAuthorsSamples() {
        let mode = StorytopiaContentMode(status: .signedIn, isSampleAuthorModeEnabled: true)
    ...
    func testSignedInReadsAndWritesTheAccount() {
        let mode = StorytopiaContentMode(status: .signedIn, isSampleAuthorModeEnabled: false)
    ...
    func testLoadingIsNotSignedOut() {
        let mode = StorytopiaContentMode(status: .loading, isSampleAuthorModeEnabled: false)
    ...
    func testMisconfiguredIsNotSignedOut() {
        let mode = StorytopiaContentMode(
    ...
    func testUserLoadIdentityChangesWithAccount() {
        let mode = StorytopiaContentMode.user
    ...
    func testSampleBrowsingLoadIdentityIgnoresAccount() {
        XCTAssertEqual(
            StorytopiaContentMode.sampleBrowsing.loadIdentity(userID: UUID()),
            StorytopiaContentMode.sampleBrowsing.loadIdentity(userID: nil)
        )
    }
    ...
    func testGateRefusesWithoutPresentingWhenSignInCannotHelp() {
        for mode in [StorytopiaContentMode.loading, .unavailable("no credentials")] {
            let gate = SignInGate()
    ...
    private func storyboard(
        ...
        GeneratedStoryboard(
            ...
            sourcePhotoCount: 0,
            createdAt: createdAt,
            storagePath: "storytopia-first-run/sample.png",
            isPrimary: isPrimary,
